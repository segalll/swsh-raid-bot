exports.run = async (client, message) => {
  ...
  const cmdUsage = client.commands.get('killraid', 'help.usage');

  if (message.mentions.channels.first()) {
    const raidKey = client.raids.findKey((r) => r.channel == message.mentions.channels.first().id && r.guild == message.guild.id);
    if (!raidKey) {
      return message
    ...
    const raid = client.raids.get(raidKey);
    let mess;
    ...
    const chan = client.channels.get(raid.channel);
  ...
  if (message.mentions.members.first()) {
    const dude = message.mentions.members.first().id;
    ...
    mess = await client.channels
      ...
      .catch(() => (mess = false));
    if (mess) await mess.delete();
    const chan = client.channels.get(client.raids.get(client.userRaids.get(dude)).channel);
    if (chan) await chan.delete();
