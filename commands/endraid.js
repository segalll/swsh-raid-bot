exports.run = async (client, message) => {
  ...
  mess = await client.channels
    .get(client.botGuilds.get(
      client.raids.get(
        client.userRaids.get(message.author.id),
      ).guild,
    ).raidChannel)
    .fetchMessage(client.userRaids.get(message.author.id))
    .catch(() => (mess = false));
  if (mess) await mess.delete();
  const chan = client.channels.get(
    client.raids.get(client.userRaids.get(message.author.id)).channel,
  );
  if (chan) await chan.delete();
  client.raids.delete(client.userRaids.get(message.author.id));
  client.guildRaids.remove(
    message.guild.id, client.userRaids.get(message.author.id),
  );
