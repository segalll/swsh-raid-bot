exports.run = async (client, message, args) => {
  ...
  const cmdUsage = client.commands.get('raid', 'help.usage');
  const setfcUsage = client.commands.get('setfc', 'help.usage');
  const raidChannelUsage = client.commands.get('raidchannel', 'help.usage');
  ...
  const raidMessage = await client.channels.get(client.botGuilds.get(message.guild.id).raidChannel).send({
    embed: {
  ...
  client.raids.set(raidMessage.id, {
    people: [message.author.id], code: args[1], overflow: [], channel: raidChannel.id, guild: message.guild.id,
  });
